	...
	public void sawOpcode(int seen) {
		...
		if (!sawOpenInAppendMode) {
			if (calledClassName.equals("java/io/ObjectOutputStream") && calledMethodName.equals("<init>")
				...
				bugReporter.reportBug(DetectorUtil.addClassAndMethod(new BugInstance(this, "IO_APPENDING_TO_OBJECT_OUTPUT_STREAM", Priorities.HIGH_PRIORITY), this).
		...
		if (calledClassName.equals("java/io/FileOutputStream") && calledMethodName.equals("<init>")
			...
			// do nothing
		        ...
		        && calledMethodSig.equals("(Ljava/io/OutputStream;)V")) {
			bugReporter.reportBug(DetectorUtil.addClassAndMethod(new BugInstance(this, "IO_APPENDING_TO_OBJECT_OUTPUT_STREAM", Priorities.HIGH_PRIORITY), this).add(AnnotationFactory.createSourceLine(this)));
			sawOpenInAppendMode = false;
		} else
			sawOpenInAppendMode = false;
