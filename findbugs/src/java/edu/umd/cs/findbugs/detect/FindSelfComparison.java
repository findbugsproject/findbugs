	...
	public void sawOpcode(int seen) {
		...
		if (seen == PUTFIELD) {
			...
			checkPUTFIELD: if (putFieldPC + 10 > getPC()
				...
				BugInstance bug = DetectorUtil.addClassAndMethod(new BugInstance(this, "SA_FIELD_DOUBLE_ASSIGNMENT", priority), this)
				.add(AnnotationFactory.createReferencedField(this));
				if (intendedTarget != null) {
					bug.add(AnnotationFactory.createField(intendedTarget))
	...
	private void checkForSelfOperation(int opCode, String op) {
		{
			...
			if (field0 != null && field0.equals(field1) && fr0 != -1 && fr0 == fr1) {
				bugAccumulator.accumulateBug(DetectorUtil.addClassAndMethod(new BugInstance(this,
						"SA_FIELD_SELF_" + op, NORMAL_PRIORITY), this).add(AnnotationFactory.createField(field0)),
						AnnotationFactory.createSourceLine(this));
			} else if (opCode == IXOR && item0.equals(item1)) {
				ILocalVariableAnnotation localVariableAnnotation = AnnotationFactory
				.createVariable(this, item0);
				if (localVariableAnnotation != null) {
					bugAccumulator.accumulateBug(DetectorUtil.addClassAndMethod(new BugInstance(this,
							"SA_LOCAL_SELF_" + op,  HIGH_PRIORITY), this).add(
									localVariableAnnotation), AnnotationFactory.createSourceLine(this));
				}
			} else if (opCode == ISUB  && registerLoadCount >= 2) { // let FindSelfComparison2 report this; more accurate
				bugAccumulator.accumulateBug(DetectorUtil.addClassAndMethod(new BugInstance(this,
						"SA_LOCAL_SELF_" + op, (opCode == ISUB || opCode == LSUB  || opCode == INVOKEINTERFACE || opCode == INVOKEVIRTUAL) ? NORMAL_PRIORITY : HIGH_PRIORITY), this).add(
								AnnotationFactory
								.createVariable(
										getMethod(), whichRegister, getPC(),
										getPC() - 1)),AnnotationFactory.createSourceLine(this));
			}
		}
	}
}
